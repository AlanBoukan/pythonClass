...
def makeShape(numShape, numEdge, forwardNumber):
    ...
    # MAKE SCREEN FROM TURTLE
    ts = t1.getscreen()
    ts.getcanvas().postscript(file="drawing2.eps")  # Save as EPS (vector format)
    
    # Convert EPS to JPG/PNG using PIL (Pillow)
    

    img = Image.open('drawing2.eps')
    img.save('drawing2.jpg')   # Save as JPG
    img.save('drawing2.png')   # Or save as PNG
